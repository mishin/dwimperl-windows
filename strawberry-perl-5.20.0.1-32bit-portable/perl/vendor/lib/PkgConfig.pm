...
if($^O =~ /^(gnukfreebsd|linux)$/ && -r "/etc/debian_version") {
    ...
    @DEFAULT_SEARCH_PATH = ();
    my @l = split /\s+/, $Config{libpth};
    for (@l) { push @DEFAULT_SEARCH_PATH, "$_/pkgconfig" if -d "$_/pkgconfig" }
}

my @ENV_SEARCH_PATH = split($Config{path_sep}, $ENV{PKG_CONFIG_PATH} || "");

unshift @DEFAULT_SEARCH_PATH, (grep { $_ !~ /[\r\n\t]/ && -d $_ } @ENV_SEARCH_PATH);
...
sub assign_var {
    ...
    if ($Config{myuname} =~ /Win32 strawberry/) {
      #XXX-UGLY-HACK strawberry perl workaround
      my $strawberry_prefix = $^X;
      $strawberry_prefix =~ s|\\|/|g;
      $strawberry_prefix =~ s|perl/bin/perl.exe|c|g;
      $value =~ s|/mingw|$strawberry_prefix|g;
    }
